...
def read_movie_item(movie_id: int, db: Session = Depends(get_db)):
    db_movie = crud.get_movie(db, movie_id=movie_id)
    return db_movie
    
    
@app.delete('/movies/{movie_id}', status_code=204)
def delete_movie_item(movie_id: int, db: Session = Depends(get_db)):
    crud.delete_movie_by_id(db, movie_id=movie_id)
    return { 'message': 'successfully deleted.' }


@app.put('/movies/{movie_id}', response_model=schemas.Movie)
def update_movie_item(movie_id: int, movie: schemas.MovieUpdate, db: Session = Depends(get_db)):
